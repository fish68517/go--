<template>
  <div id="screenPage">
    <header>
      <h4 class="title">智慧中药房监管数据全景图</h4>
      <div class="time-box">
      ...
      <div class="wrap-center">
        <div class="map-wrap">
          ...
          <div class="top-info">
            <p class="info">
              今日接方数量:
              <span>{{this.$store.state.totalData[0].produceTotal}}</span>个
            </p>
            <p class="info">
              今日完成数量:
              <span>{{this.$store.state.totalData[0].useTotal}}</span>个
            </p>
          </div>
          <!-- 滚屏数据展示 -->
          <div class="list-wrap">
...
<!--          <Maps></Maps>-->
        ...
        <div class="bottom-wrap">
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">接方</p>
              <p class="info-text">{{this.$store.state.totalData[0].modalSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">审方</p>
              <p class="info-text">{{this.$store.state.totalData[0].detonatorSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">调剂</p>
              <p class="info-text">{{this.$store.state.totalData[0].equipmentSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">复核</p>
              <p class="info-text">{{this.$store.state.totalData[0].materialSale}}个</p>
            </div>
          </div>

        </div>
        <div class="bottom-wrap_tj">
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">泡药</p>
              <p class="info-text">{{this.$store.state.totalData[0].modalSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">煎药</p>
              <p class="info-text">{{this.$store.state.totalData[0].detonatorSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">包装</p>
              <p class="info-text">{{this.$store.state.totalData[0].equipmentSale}}个</p>
            </div>
          </div>
          <div class="item-info">
            <div class="info-wrap">
              <p class="info-title">发货</p>
              <p class="info-text">{{this.$store.state.totalData[0].materialSale}}个</p>
            </div>
          </div>
...
export default {
  ...
  computed:{
    ajaxData(){
      console.log("large-ajaxData--computed"+this.timeString);
      return this.$store.state.ajaxData
    }
  },
  watch:{
    ajaxData: {
      handler(newValue, oldValue) {
        if (JSON.stringify(newValue) != JSON.stringify(oldValue)) {
          if(newValue.overviewData){
            console.log("ajaxData-overviewData--watch",newValue.overviewData);
            this.getOverviewData(newValue.overviewData);
          }
          if(newValue.marketData){
            if( Array.isArray(newValue.marketData) && newValue.marketData.length>0 ){
              for(let i=0; i<newValue.marketData.length; i++){
                let obj = newValue.marketData[i];
                if(obj.data.length>0){
                  obj.data.sort((a,b)=>{
                    return a.title.split('年')[0]-b.title.split('年')[0];
                  });
                  let num = obj.data.length;
                  obj.data = obj.data.slice(0,num);
                }
                newValue.marketData[i] = obj;
              }
            }
            this.getMarketData(newValue.marketData);
          }
          if(newValue.useData){
            let params = newValue.useData;
            let datas = [];
            ...
            this.getUseData(datas);
          }
          if(newValue.customerData){
            this.getCustomerData(newValue.customerData);
          }
          if(newValue.produceData){
            if( Array.isArray(newValue.produceData) && newValue.produceData.length>0 ){
              for(let i=0; i<newValue.produceData.length; i++){
                let obj = newValue.produceData[i];
                if(obj.data.length>0){
                  // obj.data.sort((a,b)=>{
                ...
                newValue.produceData[i] = obj;
              }
            }
            this.getProduceData(newValue.produceData);
          }
          if(newValue.saleData){
            if( Array.isArray(newValue.saleData) && newValue.saleData.length>0 ){
              for(let i=0; i<newValue.saleData.length; i++){
                let obj = newValue.saleData[i];
                if(obj.data.length>0){
                  obj.data.sort((a,b)=>{
                    return (new Date(a.title).getTime())-(new Date(b.title).getTime())
                  });
                ...
                newValue.saleData[i] = obj;
              }
            }
            this.getSaleData(newValue.saleData);
          }
          if(newValue.mapData){
            console.log("ajaxData--mapData-watch",newValue.mapData);
            this.getMapData(newValue.mapData);
          }
          if(newValue.totalData){
            console.log("ajaxData--totalData-watch",newValue.totalData);
            this.getTotalData(newValue.totalData);
          }
        }
      },
      deep: true
  ...
  methods: {
    ...
    ...mapActions({
      getBigScreen: 'actionBigScreenData',//请求大屏数据
      ...
      getProduceData: 'actionProduceData',//请求获取生产监控数据
