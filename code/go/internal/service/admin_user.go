...
func (svc *Service) CheckAdminPwdReturnUserInfo(ctx context.Context, params *dto.CheckAppPwdRequest) (error error, adminUser *model.AdminUser) {
	// 查询信息
	...
	if pwd != user.Password {
		return errors.New("密码不正确"), nil
	}
	return nil, user
}
func (svc *Service) GetUserRoleView(ctx context.Context, userId int) (*vo.AdminUserInfoVo, error) {
	// 查询信息
...
func (svc *Service) GetGetAdminUserWithRole(ctx context.Context, userId int) (*model.AdminUserWithRole, error) {
	// 查询信息
	userInfo, err := svc.dao.GetAdminUserWithRole(ctx, &model.AdminUserWithRole{ID: userId})
	if err != nil {
		return nil, err
	}
	return userInfo, nil
