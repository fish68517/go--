...
var AdjustmentHandler = new(adjustmentHandler)

func (c *adjustmentHandler) Index(ctx *gin.Context) {
	response := app.NewResponse(ctx)
	response.BuildTpl(ctx, "adjustment_index.html").WriteTpl(gin.H{})
}
func (c *adjustmentHandler) List(ctx *gin.Context) {
	response := app.NewResponse(ctx)
	params := &dto.GetAdjustListRequest{}
	...
	list, total, err := svc.GetAdjustmentList(svc.GetCtx(), params)
	if err != nil {
