...
func (c *packHandler) Index(ctx *gin.Context) {
	response := app.NewResponse(ctx)
	response.BuildTpl(ctx, "medicine_packing_index.html").WriteTpl(gin.H{})
}
func (c *packHandler) List(ctx *gin.Context) {
	response := app.NewResponse(ctx)
	params := &dto.GetPackInfoListRequest{}
	...
	list, total, err := svc.GetPackList(svc.GetCtx(), params)
	if err != nil {
