...
type AdminUser struct {
	*Model
	...
	Token     string `gorm:"column:app_token" json:"token"`                          // 明确指定数据库列名

}
type AdminUserWithRole struct {
	ID       int    `db:"id"`
...
func (l *AdminUserWithRole) TableName() string {
	return "oh_admin_user_role_view"
}
func (a *AdminUser) TableName() string {
	return global.DatabaseSetting.TablePrefix + "admin_user"
}
func (a *AdminUserWithRole) GetUserRoleView(ctx context.Context, db *gorm.DB) (*AdminUserWithRole, error) {
	var admin *AdminUserWithRole
